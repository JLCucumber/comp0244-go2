...
if __name__ == "__main__":
    # 生成星形 & 柔和凹形障碍物
    star1 = generate_star(center=(1, 1), size=1)
    star2 = generate_star(center=(3, 3), size=1.5)
    # soft_concave1 = generate_soft_concave(center=(0, 0), size=1)

    # 存储到 JSON
    name = "map_with_complex_obstacles.json"
    save_map_to_json([star1, star2], name)
    # save_map_to_json([soft_concave1], "map_with_complex_obstacles_1.json")
    print(f"地图已成功保存到 {name}")
